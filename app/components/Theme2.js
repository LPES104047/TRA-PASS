  ...
  return (
    <>
     ...
     .theme2-root .train-item::before {
       ...
       left: 57px; width: 14px; height: 14px;
       border-radius: 50%;
     ...
     @media (min-width: 768px) {
       ...
       .theme2-root .time-label { left: -10px; width: 60px; font-size: 18px; }
       .theme2-root .dashboard::before { left: 65px; width: 4px; }
       .theme2-root .train-item::before { left: 56px; width: 18px; height: 18px; border-width: 2px; }
       
       .theme2-root .card { padding: 25px 155px 25px 35px; }
          ...
          <div className="ticket-body">
            <div className="dashboard">
              {validTrains.length === 0 ? (
                ...
                validTrains.map((t) => {
                  const diffMins = t.waitMins;
                  const percent = Math.max(0, Math.min(100, 100 - (diffMins / 120 * 100)));
                  let waitText = diffMins >= 60 ? `${Math.floor(diffMins/60)} Hr ${diffMins%60} Min` : `${diffMins} Min`;
